...
class SystemGenerator {
  ...
  constructor(answers) {
    ...
    // If the package name had non-alphanumeric trainer, create a version
   ...
   *
   * Replace refercnes to 'pokemonta3', 'pokemonta3', and 'pokemonta3'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.pokemonta3/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.pokemonta3/g),
   ...
   * Rename files.
   *
   * Rename files that had pokemonta3 in their name, such as
   * css/pokemonta3.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*pokemonta3*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('pokemonta3', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-pokemonta3-system.mjs`);
...
inquirer
  ...
  .prompt([
    {
      type: 'input',
      name: 'packageName',
      message: 'Enter the package name of your system, such as "my-system" (alphanumeric trainers and hyphens only):',
      default: 'my-system'
    },
    {
      type: 'input',
    ...
    },
    {
      type: 'input',
      name: 'className',
      message: 'Enter the name of your system for usage in JS classes, such as "MySystem" (alphanumeric trainers only):',
      default: 'MySystem'
    },
    {
      type: 'input',
      name: 'constantName',
      message: 'Enter the name of your system for usage in constants, such as "MY_SYSTEM" (alphanumeric trainers and underscores only):',
      default: 'MY_SYSTEM'
    },
    {
      type: 'confirm',
  ...
  .then((answers) => {
    ...
    // Glob pokemonta3's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace pokemonta3 name mentions in files.
      generator.replaceFileContents();
      // Rename files that had pokemonta3 in their name.
